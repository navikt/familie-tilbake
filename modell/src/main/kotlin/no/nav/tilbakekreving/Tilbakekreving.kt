...
class Tilbakekreving internal constructor(
    ...
    // Denne må fjernes etter at eksisterende data er migrert
    fun migrerBigQueryData() {
        if (!behandlingHistorikk.harBehandling()) return
        bigQueryService.oppdaterBehandling(
            behandlingHistorikk.nåværende().entry.bigqueryData(tilstand, eksternFagsak.ytelse.hentYtelsesnavn(Språkkode.NB)),
        )
    }
