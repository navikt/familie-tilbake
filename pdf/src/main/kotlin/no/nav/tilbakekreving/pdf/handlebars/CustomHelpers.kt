...
import no.nav.tilbakekreving.pdf.DatoUtil.DATO_FORMAT_DATO_MÅNEDSNAVN_ÅR
...
class DatoHelper : Helper<Any> {
    ...
    override fun apply(
        ...
        val date = ObjectMapperForUtvekslingAvDataMedHandlebars.INSTANCE.convertValue(context, LocalDate::class.java)
...
class MånedHelper : Helper<Any> {
    ...
    override fun apply(
        ...
        val date = ObjectMapperForUtvekslingAvDataMedHandlebars.INSTANCE.convertValue(context, YearMonth::class.java)
        return format.format(date)
    }
}
