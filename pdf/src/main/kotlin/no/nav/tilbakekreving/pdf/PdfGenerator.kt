...
class PdfGenerator {
    ...
    private fun fontSupplier(fontName: String): PDFontSupplier {
        ...
        val font = TTFParser().parse(RandomAccessReadBuffer(PdfGenerator::class.java.getResourceAsStream("/fonts/$fontName"))).also { it.isEnableGsub = false }
    ...
    private fun hentCss(format: DocFormat): String =
        FileStructureUtil.readResourceAsString("formats/" + format.name.lowercase(Locale.getDefault()) + "/style.css")
}
