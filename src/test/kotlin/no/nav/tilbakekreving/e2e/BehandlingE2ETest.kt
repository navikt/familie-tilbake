...
class BehandlingE2ETest : TilbakekrevingE2EBase() {
    ...
    fun `endringer i behandling skal føre til kafka-meldinger til dvh`() {
        ...
        dvhHendelser[1].ansvarligSaksbehandler shouldBe SAKSBEHANDLER_IDENT
        ...
        dvhHendelser.last().ansvarligSaksbehandler shouldBe SAKSBEHANDLER_IDENT
        ...
        dvhHendelser[5].ansvarligSaksbehandler shouldBe SAKSBEHANDLER_IDENT
        dvhHendelser[5].ansvarligBeslutter shouldBe BESLUTTER_IDENT
        ...
        dvhHendelser[6].ansvarligSaksbehandler shouldBe SAKSBEHANDLER_IDENT
        dvhHendelser[6].ansvarligBeslutter shouldBe BESLUTTER_IDENT
        ...
        vedtaksoppsummering.ansvarligSaksbehandler shouldBe SAKSBEHANDLER_IDENT
        vedtaksoppsummering.ansvarligBeslutter shouldBe BESLUTTER_IDENT
    ...
    fun `slå sammen vilkårsvurderingsperioder etter splitting`() {
        ...
        val perioder = somSaksbehandler(SAKSBEHANDLER_IDENT) {
    ...
    fun `trenger ny vurdering av steg blir lagret`() {
        ...
        val behandling = somSaksbehandler(SAKSBEHANDLER_IDENT) { behandlingController.hentBehandling(behandlingId).data.shouldNotBeNull() }
    ...
    @Test
    fun `godTro, deler av beløpet i behold ny vilkårsvurdering, med reduksjon`() {
        val fagsystemId = KravgrunnlagGenerator.nextPaddedId(6)
        val perioder = listOf(
            KravgrunnlagGenerator.standardPeriode(1.januar(2021) til 1.januar(2021)),
            KravgrunnlagGenerator.standardPeriode(1.februar(2021) til 1.februar(2021)),
        ...
        somSaksbehandler(SAKSBEHANDLER_IDENT) {
            behandlingApiController.behandlingLagreVilkaarsvurdering(
                ...
                vilkaarsvurderingDto = VilkaarsvurderingDto(
                    ...
                    delbarePerioder = vilkårvsurderingPerioder.first().vilkårsvurdering.delbarePerioder,
                    valg = no.nav.tilbakekreving.kontrakter.frontend.models.GodTroDto(
                        beløpIBehold = DelerDto(
                            beløp = 500,
                            begrunnelse = "Begrunnelse for deler av beløpet er i behold",
                            reduksjon = SkalReduseresDto(
                                relevans = listOf(
                                    MomentDto(
                                        moment = "ANNET",
        ...
        vilkårvsurderingPerioderEtter.first().vilkårsvurdering.valg.shouldBeInstanceOf<no.nav.tilbakekreving.kontrakter.frontend.models.GodTroDto> {
            ...
            it.beløpIBehold.shouldBeInstanceOf<DelerDto> { deler ->
                deler.begrunnelse shouldBe "Begrunnelse for deler av beløpet er i behold"
    ...
    @Test
    fun `godTro, deler av beløpet i behold ny vilkårsvurdering, uten reduksjon`() {
        val fagsystemId = KravgrunnlagGenerator.nextPaddedId(6)
        val perioder = listOf(
            KravgrunnlagGenerator.standardPeriode(1.januar(2021) til 1.januar(2021)),
            KravgrunnlagGenerator.standardPeriode(1.februar(2021) til 1.februar(2021)),
    ...
    @Test
    fun `godTro, ikke i behold ny vilkårsvurdering`() {
        val fagsystemId = KravgrunnlagGenerator.nextPaddedId(6)
        val perioder = listOf(
            KravgrunnlagGenerator.standardPeriode(1.januar(2021) til 1.januar(2021)),
            KravgrunnlagGenerator.standardPeriode(1.februar(2021) til 1.februar(2021)),
        ...
        somSaksbehandler(SAKSBEHANDLER_IDENT) {
            behandlingApiController.behandlingLagreVilkaarsvurdering(
                ...
                vilkaarsvurderingDto = VilkaarsvurderingDto(
                    ...
                    delbarePerioder = vilkårvsurderingPerioder.first().vilkårsvurdering.delbarePerioder,
                    valg = no.nav.tilbakekreving.kontrakter.frontend.models.GodTroDto(
                        beløpIBehold = IngentingDto(
                            begrunnelse = "Begrunnelse for at beløpet ikke er i behold",
                        ),
                        begrunnelse = "Begrunnelse i GodTro",
                    ),
                ),
        ...
        vilkårvsurderingPerioderEtter.first().vilkårsvurdering.valg.shouldBeInstanceOf<no.nav.tilbakekreving.kontrakter.frontend.models.GodTroDto> {
            it.begrunnelse shouldBe "Begrunnelse i GodTro"
            it.beløpIBehold.shouldBeInstanceOf<IngentingDto> {
                it.begrunnelse shouldBe "Begrunnelse for at beløpet ikke er i behold"
            }
        }
    }
}
