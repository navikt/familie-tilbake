...
class AvstemmingsfilTest {
    ...
    fun `tilFlatfil skal ha newline for å skille rader`() {
        val avstemmingsfil = Avstemmingsfil(listOf(testRad(), testRad()))
    ...
    fun `tilFlatfil skal bruke kode i siste kolonne når det er omgjøring til ingen tilbakekreving`() {
        val avstemmingsfil = Avstemmingsfil(listOf(Rad(avsender = "fptilbake",
                                                       vedtakId = "1234",
                                                       fnr = "12345678901",
