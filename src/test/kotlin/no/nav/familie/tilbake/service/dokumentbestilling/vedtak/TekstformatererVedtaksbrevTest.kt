...
class TekstformatererVedtaksbrevTest {
    ...
    @Test
    fun `lagVedtaksbrevFritekst skal generere vedtaksbrev for revurdering med OS og mye fritekst`() {
        val vedtaksbrevData = felles.copy(fagsaksvedtaksdato = LocalDate.now(),
                                          behandling = HbBehandling(erRevurdering = true,
                                                                    originalBehandlingsdatoFagsakvedtak = LocalDate.of(2019,
                                                                                                                       1,
                                                                                                                       1)),
                                          totalresultat = HbTotalresultat(Vedtaksresultat.DELVIS_TILBAKEBETALING,
                                                                          ...
                                                                          BigDecimal.ZERO),
                                          hjemmel = HbHjemmel("Folketrygdloven § 22-15"),
                                          varsel = HbVarsel(varsletBeløp = BigDecimal(1234567893),
                ...
                listOf(HbVedtaksbrevsperiode(periode = januar,
                                             ...
                                             HbVurderinger(foreldelsevurdering = Foreldelsesvurderingstype.IKKE_VURDERT,
                                                           ...
                                                           HbSærligeGrunner(listOf(SærligGrunn.HELT_ELLER_DELVIS_NAVS_FEIL,
                                                                            ...
                                                                            " beløp! Du skal slippe å betale renter, for det " +
                                                                            "har du ikke råd til uansett!",
    ...
    @Test
    fun `lagVedtaksbrevFritekst skal generere vedtaksbrev for OS og ett barn og forsett`() {
        val vedtaksbrevData = felles
                .copy(fagsaksvedtaksdato = LocalDate.now(),
                      ...
                      hjemmel = HbHjemmel("Folketrygdloven § 22-15"),
                      varsel = HbVarsel(varsletBeløp = BigDecimal(10000),
    ...
    fun `lagVedtaksbrevFritekst skal generere vedtaksbrev for_KS_og forsett`() {
        val vedtaksbrevData = felles
                .copy(brevmetadata = brevmetadata.copy(ytelsestype = Ytelsestype.KONTANTSTØTTE),
                      fagsaksvedtaksdato = LocalDate.now(),
                      totalresultat = HbTotalresultat(hovedresultat = Vedtaksresultat.FULL_TILBAKEBETALING,
                                                      totaltTilbakekrevesBeløp = BigDecimal(10000),
    ...
    fun `skal generere vedtaksbrev for OS med og uten foreldelse og uten skatt`() {
        val vedtaksbrevData = felles
                .copy(fagsaksvedtaksdato = LocalDate.of(2019, 11, 12),
        ...
        val perioder: List<HbVedtaksbrevsperiode> =
                listOf(HbVedtaksbrevsperiode(periode = januar,
                                             ...
                                             resultat = HbResultat(tilbakekrevesBeløp = BigDecimal.ZERO,
                                                                   tilbakekrevesBeløpUtenSkattMedRenter = BigDecimal.ZERO,
                                                                   rentebeløp = BigDecimal.ZERO,
                                                                   foreldetBeløp = BigDecimal(1000))),
                       HbVedtaksbrevsperiode(periode = februar,
                                             vurderinger =
                                             HbVurderinger(foreldelsevurdering = Foreldelsesvurderingstype.TILLEGGSFRIST,
                                                           foreldelsesfrist = januar.fom.plusMonths(11),
    ...
    @Test
    fun `lagVedtaksbrevFritekst skal generere vedtaksbrev for OS ingen tilbakekreving pga lavt beløp`() {
        val vedtaksbrevData = felles
                .copy(fagsaksvedtaksdato = LocalDate.now(),
                      totalresultat = HbTotalresultat(hovedresultat = Vedtaksresultat.INGEN_TILBAKEBETALING,
                      ...
                      varsel = HbVarsel(varsletBeløp = BigDecimal(500),
                                        varsletDato = LocalDate.of(2020, 4, 4)),
                      vedtaksbrevstype = Vedtaksbrevstype.ORDINÆR)
        val perioder: List<HbVedtaksbrevsperiode> =
    ...
    @Test
    fun `lagVedtaksbrevFritekst skal generere vedtaksbrev for OS ingen tilbakekreving pga lavt beløp med korrigert beløp`() {
        val vedtaksbrevData = felles
                .copy(fagsaksvedtaksdato = LocalDate.now(),
                      totalresultat = HbTotalresultat(hovedresultat = Vedtaksresultat.INGEN_TILBAKEBETALING,
                                                      totaltTilbakekrevesBeløp = BigDecimal.ZERO,
                                                      ...
                                                      totaltTilbakekrevesBeløpMedRenterUtenSkatt = BigDecimal.ZERO),
                      hjemmel = HbHjemmel("Folketrygdloven § 22-15 6.ledd"),
                      varsel = HbVarsel(varsletBeløp = BigDecimal(15000),
                                        varsletDato = LocalDate.of(2020, 4, 4)),
    ...
    @Test
    fun `lagVedtaksbrevOverskrift skal generere vedtaksbrev overskrift_OVERGANGSSTØNAD_full tilbakebetaling`() {
    ...
    @Test
    fun `lagVedtaksbrevOverskrift skal generere vedtaksbrev overskrift_kontantstøtte_full tilbakebetaling nynorsk`() {
        val data = lagBrevOverskriftTestoppsett(Ytelsestype.KONTANTSTØTTE,
    ...
    @Test
    fun `lagVedtaksbrevOverskrift skal generere vedtaksbrev overskrift_KONTANTSTØTTE_ingen tilbakebetaling`() {
        val data: HbVedtaksbrevsdata =
    ...
    @Test
    fun `lagDeltekst skal ha riktig tekst for særlige grunner når det er reduksjon av beløp`() {
        val felles = felles.copy(brevmetadata = brevmetadata.copy(språkkode = Språkkode.NN),
                                 fagsaksvedtaksdato = LocalDate.now(),
                                 totalresultat = HbTotalresultat(Vedtaksresultat.FULL_TILBAKEBETALING,
                                                                 BigDecimal(1000),
                                                                 BigDecimal(1100),
                                 ...
                                 varsel = HbVarsel(varsletBeløp = BigDecimal(1000),
                                                   varsletDato = LocalDate.of(2020, 4, 4)))
        val periode =
                HbVedtaksbrevsperiode(periode = januar,
                                      ...
                                      vurderinger = HbVurderinger(foreldelsevurdering = Foreldelsesvurderingstype.IKKE_VURDERT,
                                                                  ...
                                                                  HbSærligeGrunner(listOf(SærligGrunn.GRAD_AV_UAKTSOMHET),
                                                                                   null,
                                                                                   null)),
                                      resultat = HbResultat(tilbakekrevesBeløp = BigDecimal(500),
    ...
    @Test
    fun `lagDeltekst skal generere tekst for faktaperiode`() {
        val felles = felles.copy(fagsaksvedtaksdato = LocalDate.now(),
                                 totalresultat = HbTotalresultat(Vedtaksresultat.DELVIS_TILBAKEBETALING,
                                                                 BigDecimal(23002),
                                                                 BigDecimal(23002),
                                                                 BigDecimal(23002),
                                                                 BigDecimal.ZERO),
                                 hjemmel = HbHjemmel("foo"),
                                 datoer = HbVedtaksbrevDatoer(opphørsdatoIkkeOmsorg = LocalDate.of(2020,
                                                                                                   4,
                                                                                                   4)),
    ...
    @Test
    fun `lagDeltekst skal si at du ikke trenger betale tilbake når det er god tro og beløp ikke er i behold`() {
        val felles = felles.copy(fagsaksvedtaksdato = LocalDate.now(),
                                 totalresultat = HbTotalresultat(Vedtaksresultat.DELVIS_TILBAKEBETALING,
                                                                 BigDecimal.ZERO,
                                                                 BigDecimal.ZERO,
    ...
    @Test
    fun `lagDeltekst skal ha riktig tekst for særlige grunner når det ikke er reduksjon av beløp`() {
        val felles = felles.copy(fagsaksvedtaksdato = LocalDate.now(),
                                 totalresultat = HbTotalresultat(Vedtaksresultat.FULL_TILBAKEBETALING,
                                                                 BigDecimal(1000),
                                                                 BigDecimal(1100),
        ...
        val periode =
                HbVedtaksbrevsperiode(periode = januar,
                                      ...
                                      vurderinger = HbVurderinger(foreldelsevurdering = Foreldelsesvurderingstype.IKKE_VURDERT,
                                                                  ...
                                                                  særligeGrunner =
                                                                  HbSærligeGrunner(listOf(SærligGrunn.GRAD_AV_UAKTSOMHET))),
                                      resultat = HbResultat(tilbakekrevesBeløp = BigDecimal(1000),
    ...
    private fun lagBrevOverskriftTestoppsett(ytelsestype: Ytelsestype,
        ...
        return HbVedtaksbrevsdata(felles.copy(brevmetadata = brevmetadata.copy(språkkode = språkkode, ytelsestype = ytelsestype),
                                              totalresultat = felles.totalresultat.copy(hovedresultat = hovedresultat)),
                                  emptyList())
    }

    private fun lagFritekstVedtaksbrevData(ytelsestype: Ytelsestype,
                                           hovedresultat: Vedtaksresultat): HbVedtaksbrevsdata {
        return HbVedtaksbrevsdata(felles.copy(brevmetadata = brevmetadata.copy(språkkode = Språkkode.NB,
                                              ...
                                              vedtaksbrevstype = Vedtaksbrevstype.FRITEKST_FEILUTBETALING_BORTFALT), emptyList())
    }
