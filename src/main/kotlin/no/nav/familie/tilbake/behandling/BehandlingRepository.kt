...
interface BehandlingRepository : RepositoryInterface<Behandling, UUID>, InsertUpdateRepository<Behandling> {
    ...
    // language=PostgreSQL
    @Query("select beh from behandling beh inner join fagsak f on beh.fagsak_id = f.id " +
           "where f.ytelsestype=:fagsystem and f.ekstern_fagsak_id=:eksternFagsakId " +
           "and beh.status <>'AVSLUTTET' and beh.type='TILBAKEKREVING'")
    fun finnÅpenTilbakekrevingsbehandling(fagsystem: String,
                                          eksternFagsakId: String): Behandling?

    // language=PostgreSQL
    @Query("select beh from behandling beh inner join ekstern_behandling eks on eks.behandling_id= beh.id " +
