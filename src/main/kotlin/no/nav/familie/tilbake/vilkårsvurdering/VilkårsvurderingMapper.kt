...
object VilkårsvurderingMapper {
    ...
    fun tilRespons(vilkårsvurdering: Vilkårsvurdering?,
                   perioder: List<Periode>,
                   foreldetPerioder: List<Periode>,
                   faktaFeilutbetaling: FaktaFeilutbetaling,
                   kravgrunnlag431: Kravgrunnlag431): VurdertVilkårsvurderingDto {
        //Når perioder er allerede behandlet
        val vilkårsvurdertePerioder = vilkårsvurdering?.perioder?.map {
        ...
        val samletPerioder = foreldetPerioder.map {
            VurdertVilkårsvurderingsperiodeDto(
                    periode = PeriodeDto(it),
                    feilutbetaltBeløp = beregnFeilutbetaltBeløp(kravgrunnlag431, it),
                    hendelsestype = hentHendelsestype(faktaFeilutbetaling.perioder, it),
    ...
    fun tilDomene(behandlingId: UUID, vilkårsvurderingsperioder: List<VilkårsvurderingsperiodeDto>,
                  fagsystem: Fagsystem): Vilkårsvurdering {
        val vilkårsvurderingsperiode = vilkårsvurderingsperioder.map {
            Vilkårsvurderingsperiode(periode = Periode(it.periode.fom, it.periode.tom),
                                     begrunnelse = it.begrunnelse,
    ...
    private fun tilGodTroDto(vilkårsvurderingGodTro: VilkårsvurderingGodTro?): VurdertGodTroDto? {
        if (vilkårsvurderingGodTro != null) {
            return VurdertGodTroDto(begrunnelse = vilkårsvurderingGodTro.begrunnelse,
    ...
    private fun tilDomeneGodTro(godTroDto: GodTroDto?): VilkårsvurderingGodTro? {
        if (godTroDto != null) {
            return VilkårsvurderingGodTro(begrunnelse = godTroDto.begrunnelse,
    ...
    private fun tilSærligGrunnerDto(særligGrunner: Set<VilkårsvurderingSærligGrunn>): List<VurdertSærligGrunnDto> =
            særligGrunner.map {
                VurdertSærligGrunnDto(særligGrunn = it.særligGrunn,
                                      begrunnelse = it.begrunnelse)
            }

    private fun tilSærligGrunnerDomene(særligGrunner: List<SærligGrunnDto>?): Set<VilkårsvurderingSærligGrunn> =
            særligGrunner?.map {
                VilkårsvurderingSærligGrunn(særligGrunn = it.særligGrunn,
                                            begrunnelse = it.begrunnelse)
            }?.toSet() ?: emptySet()

    private fun beregnFeilutbetaltBeløp(kravgrunnlag431: Kravgrunnlag431, periode: Periode): BigDecimal =
            KravgrunnlagsberegningService.beregnFeilutbetaltBeløp(kravgrunnlag431, periode)
                    .setScale(0, RoundingMode.HALF_UP)

    private fun hentHendelsestype(faktaPerioder: Set<FaktaFeilutbetalingsperiode>,
                                  vurdertVilkårsperiode: Periode): Hendelsestype =
            faktaPerioder.first { it.periode.overlapper(vurdertVilkårsperiode) }.hendelsestype

    private fun utledReduserteBeløp(kravgrunnlag431: Kravgrunnlag431,
                                    vurdertVilkårsperiode: Periode): List<RedusertBeløpDto> {
    ...
    private fun hentAktiviteter(kravgrunnlag431: Kravgrunnlag431,
                                vurdertVilkårsperiode: Periode): List<AktivitetDto> {
        val perioder = kravgrunnlag431.perioder.filter { vurdertVilkårsperiode.overlapper(it.periode) }
