...
interface MeldingstellingRepository : RepositoryInterface<Meldingstelling, UUID>,
    ...
    fun findByFagsystemAndTypeAndStatusAndDato(fagsystem: Fagsystem,
                                               type: Meldingstype,
    ...
    @Query("""SELECT fagsystem, dato, SUM(antall) as antall FROM meldingstelling
              WHERE type = :type
    ...
    @Query("""UPDATE meldingstelling SET antall = antall + 1 
              ...
              AND status = :status
              AND dato = :dato""")
    fun oppdaterTeller(fagsystem: Fagsystem,
                       type: Meldingstype,
                       status: Mottaksstatus,
    ...
    // language=PostgreSQL
    @Query("""SELECT fagsystem, behandlingssteg, COUNT(*) AS antall
              FROM fagsak
              JOIN behandling ON fagsak.id = behandling.fagsak_id
