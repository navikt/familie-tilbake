...
class FilMapper(
    private val rader: List<Rad>,
) {
    fun tilFlatfil(): ByteArray = (HEADER + rader.joinToString(SKILLETEGN_RADER) { it.toCsvString() }).toByteArray()

    companion object {
        private const val SKILLETEGN_RADER = "\n"
        const val HEADER =
            "avsender" + Rad.SKILLETEGN_KOLONNER +
