...
class BigQueryAdapterService(
    ...
    fun oppdaterBigQuery(
        ...
        val periode = kravgrunnlag?.samletPeriode() ?: varsel?.perioder?.let { periode -> periode.minOf { it.fom } til periode.maxOf { it.tom } }
        bigQueryService.oppdaterBehandling(
            BigQueryBehandlingDataDto(
                behandlingId = behandling.id.toString(),
                opprettetDato = behandling.opprettetTidspunkt,
                periode = requireNotNull(periode),
                behandlingstype = behandling.type.name,
                ytelse = ytelsestype,
                beløp = requireNotNull(kravgrunnlag?.sumFeilutbetaling()?.toLong() ?: varsel?.sumFeilutbetaling?.toLong()),
