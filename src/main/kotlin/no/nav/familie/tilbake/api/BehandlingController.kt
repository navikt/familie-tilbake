...
class BehandlingController(
    ...
    private fun validerKanSetteBehandlingTilbakeTilFakta(
        ...
        feilHvis(!erAnsvarligSaksbehandler(behandling) && !tilgangService.harInnloggetBrukerForvalterRolle(), HttpStatus.FORBIDDEN, logContext) {
    ...
    private fun erAnsvarligSaksbehandler(
        behandling: BehandlingDto,
    ): Boolean = ContextService.hentSaksbehandler(SecureLog.Context.medBehandling(behandling.eksternFagsakId, behandling.behandlingId.toString())) == behandling.ansvarligSaksbehandler
}
