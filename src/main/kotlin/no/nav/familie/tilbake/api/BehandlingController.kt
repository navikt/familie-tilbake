...
class BehandlingController(val behandlingService: BehandlingService) {
    ...
    @GetMapping(path = ["/v1/{eksternbrukId}"],
                produces = [MediaType.APPLICATION_JSON_VALUE])
    fun hentBehandling(@NotNull @PathVariable("eksternbrukId") eksternBrukId: UUID): Ressurs<HentBehandlingResponsDto> {
        return Ressurs.success(behandlingService.hentBehandling(eksternBrukId))
    }
}
