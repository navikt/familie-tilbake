...
class DokarkivServiceImpl(
    ...
    override suspend fun journalførVarselbrev(
        token: JwtToken,
        varselbrevBehov: VarselbrevBehov,
        logContext: SecureLog.Context,
    ): OpprettJournalpostResponse? {
        val pdfGenerator = pdfFactory()
        println("======>>>> journalførVarselbrev: varselbrevBehov.eksternFagsakId: ${varselbrevBehov.eksternFagsakId} ")
        val brevmetadata = Brevmetadata(
            ...
            behandlendeEnhetsNavn = "Oslo", // Todo for testing harkodet Oslo for nå. må fjernes!! requireNotNull(varselbrevBehov.behandlendeEnhet) { "Enhetsnavn kreves for journalføring" }.navn,
            ansvarligSaksbehandler = "1234", // Todo for testing harkodet Oslo for nå. må fjernes!! requireNotNull(varselbrevBehov.varselbrev.ansvarligSaksbehandlerIdent) {"ansvarligSaksbehandlerIdent kreves for journalføring"},
